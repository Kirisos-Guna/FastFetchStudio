...
.PARAMETER FixExecutionPolicy
    If the execution policy is blocking scripts and profiles (Restricted or
    AllSigned) and is not enforced by Group Policy, set it to RemoteSigned for
    the current user. User scope only - no Administrator rights needed.
...
# Where the app writes its launcher, and the markers that identify it.
$LauncherPath      = Join-Path $env:USERPROFILE '.config\fastfetch\fastfetch-random.ps1'
$LauncherMarker    = '# >>> FastFetch Studio :: launcher >>>'
$BootstrapMarker   = '# >>> FastFetch Studio :: launcher (bootstrap) >>>'

# A minimal stand-in for the launcher the app generates, so that pasting the
# documented line into a profile works on a machine where FastFetch Studio has
# never been run. Without it that line fails with
#   "The term '...\fastfetch-random.ps1' is not recognized as the name of a
#    cmdlet, function, script file, or operable program."
# The app replaces this file on "Apply & Generate"; this only runs fastfetch.
$BootstrapLauncher = @'
# >>> FastFetch Studio :: launcher (bootstrap) >>>
# Placeholder written by setup.ps1 so the documented $PROFILE line works before
# FastFetch Studio has ever been run. Open the app and click "Apply & Generate"
# to replace this with the full random theme + logo launcher.
...
if (Test-Path -LiteralPath $ffExe) { & $ffExe }
# <<< FastFetch Studio :: launcher (bootstrap) <<<
'@
...
function Write-LauncherBootstrap {
    <#
    Create the launcher the profile hook looks for, when it does not exist yet.

    Returns:
      'present'   - the app already generated a real launcher; left alone
      'bootstrap' - a bootstrap file is already there; left alone
      'created'   - a bootstrap was written
      'foreign'   - a file exists that we did not write; left alone
      'skipped'   - -DryRun
    #>
    param([string]$Path)
    ...
    if (Test-Path -LiteralPath $Path -PathType Leaf) {
        $head = ''
        try { $head = [IO.File]::ReadAllText($Path) } catch { $head = '' }
        if ($head.Contains($LauncherMarker))    { return 'present' }
        if ($head.Contains($BootstrapMarker))   { return 'bootstrap' }
        return 'foreign'
    }
    ...
    # ASCII + LF: the file is pure ASCII PowerShell, and PS 5.1 reads LF fine.
    [IO.File]::WriteAllText($Path, ($BootstrapLauncher -replace "`r`n", "`n"), (New-Object System.Text.UTF8Encoding($false)))
    return 'created'
}
...
if ($Uninstall) {
    ...
    # Only remove the bootstrap this script wrote. A real launcher generated by
    # the app is the user's own configuration, so it is reported, not deleted.
    if (Test-Path -LiteralPath $LauncherPath -PathType Leaf) {
        $text = ''
        try { $text = [IO.File]::ReadAllText($LauncherPath) } catch { }
        if ($text.Contains($BootstrapMarker)) {
            if ($DryRun) {
                Say-Skip "would remove the bootstrap launcher $LauncherPath"
            } else {
                Remove-Item -LiteralPath $LauncherPath -Force
                Say-Ok "removed the bootstrap launcher $LauncherPath"
            }
        } elseif ($text.Contains($LauncherMarker)) {
            Say-Info "Launcher generated by FastFetch Studio left in place: $LauncherPath"
        }
    }
...
# ---------------------------------------------------------------- 1. binary
Say-Head 'Step 1/4 - fastfetch binary'
...
# ---------------------------------------------------------------- 3. profile
Say-Head 'Step 3/4 - PowerShell profile hook'
...
# ---------------------------------------------------------------- 4. launcher
Say-Head 'Step 4/4 - random launcher'

# The profile hook (and the line the README tells people to paste) both point at
# fastfetch-random.ps1, which is normally generated by the app's Apply button.
# On a machine where the app has never run that file does not exist, so the
# pasted line fails with "is not recognized as the name of a cmdlet, function,
# script file, or operable program". Write a bootstrap so it works immediately.
$launcherState = Write-LauncherBootstrap -Path $LauncherPath
switch ($launcherState) {
    'created'   { Say-Ok "wrote a bootstrap launcher: $LauncherPath" }
    'present'   { Say-Ok 'launcher already generated by FastFetch Studio - left alone' }
    'bootstrap' { Say-Ok 'bootstrap launcher already present - left alone' }
    'skipped'   { Say-Skip "would write a bootstrap launcher to $LauncherPath" }
    'foreign'   {
        Say-Warn "a file already exists at $LauncherPath but was not written by this tool"
        Say-Info 'Left untouched. Remove it if you want FastFetch Studio to manage that path.'
    }
}
if ($launcherState -eq 'created' -or $launcherState -eq 'bootstrap') {
    Say-Info 'Open FastFetch Studio and click "Apply & Generate" to replace it with the random theme + logo launcher.'
}


# ---------------------------------------------------------------- guardrails
# Effective policy is what decides whether a profile or a .ps1 runs at all, so
# check every scope rather than just CurrentUser.
$policy = Get-ExecutionPolicy
$gpoPolicy = @('MachinePolicy', 'UserPolicy') |
             ForEach-Object { Get-ExecutionPolicy -Scope $_ } |
             Where-Object { $_ -and $_ -ne 'Undefined' }

if ($gpoPolicy.Count -gt 0) {
    Add-Warning "Execution policy is set by Group Policy ('$($gpoPolicy -join ', ')') - this script cannot change it."
    Say-Info 'Ask your administrator to allow RemoteSigned, or run shells with -ExecutionPolicy Bypass.'
} elseif ($policy -eq 'Restricted' -or $policy -eq 'AllSigned') {
    if ($FixExecutionPolicy) {
        if ($DryRun) {
            Say-Skip "would run: Set-ExecutionPolicy -Scope CurrentUser RemoteSigned"
        } else {
            try {
                Set-ExecutionPolicy -Scope CurrentUser -ExecutionPolicy RemoteSigned -Force -ErrorAction Stop
                Say-Ok "execution policy for your user set to RemoteSigned (was '$policy')"
            } catch {
                Add-Warning "could not set the execution policy: $($_.Exception.Message)"
                Say-Info 'Run it by hand:  Set-ExecutionPolicy -Scope CurrentUser RemoteSigned'
            }
        }
    } else {
        Add-Warning "PowerShell execution policy is '$policy' - scripts and profiles will NOT run."
        Say-Info 'Fix with:  Set-ExecutionPolicy -Scope CurrentUser RemoteSigned'
        Say-Info 'Or re-run this script with -FixExecutionPolicy to apply that for you.'
    }
}
...
Say-Info "Launcher : $LauncherPath"
...
Say-Info 'Random logos/themes: run FastFetch Studio (FastFetchStudio.exe), then click Apply & Generate'
